...
run_bold_trial <- function(
  ...
  if (!is.null(tau_after_dlt)) {
    stopifnot(length(tau_after_dlt) == 1L, is.finite(tau_after_dlt),
              tau_after_dlt > 0, tau_after_dlt < 1)
  }
  ...
  repeat {
    ...
    if (!length(candidates)) {
      ...
      decision_tau <- if (sum(x) > 0L && !is.null(tau_after_dlt)) tau_after_dlt else tau
      loss <- rep(Inf, j_max)
      loss[candidates] <- abs(ppat[candidates] - decision_tau)
      chosen <- choose_tied_dose(loss, ppat, decision_tau)
    }
