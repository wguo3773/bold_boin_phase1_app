...
d <- subset(d, scenario %in% c("2", "3", "4", ">4"))
d$scenario <- factor(d$scenario, c("2", "3", "4", ">4"),
                     c("Dose 2", "Dose 3", "Dose 4", "All below target\n(>4)"))
...
p <- ggplot(d, aes(scenario, accuracy_pct, fill = method)) +
  ...
  labs(title = "MTD-selection accuracy: selected upper-dose scenarios",
